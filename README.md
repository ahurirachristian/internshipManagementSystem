...
If you prefer to add them manually via SQL, use this template and replace `<BCryptHash>` with a BCrypt hash generated by your application or another tool:

```sql
INSERT INTO users (username, password, role) VALUES
   ('student', '<BCryptHash-for-student123>', 'STUDENT'),
   ('university', '<BCryptHash-for-university123>', 'SUPERVISOR'),
   ('airtel', '<BCryptHash-for-company123>', 'COMPANY'),
   ('admin', '<BCryptHash-for-admin123>', 'ADMIN');
```

Easiest: run the backend once (`./mvnw spring-boot:run`) and the seed will ensure the demo users exist.
