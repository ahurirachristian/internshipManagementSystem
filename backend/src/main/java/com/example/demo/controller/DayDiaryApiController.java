...
public class DayDiaryApiController {
    ...
    public ResponseEntity<?> submitFeedback(@PathVariable Long id, @RequestBody Map<String, String> body) {
        ...
        if (body.containsKey("industrialSupervisorComment")) {
            diary.setIndustrialSupervisorComment(body.get("industrialSupervisorComment"));
        }
        if (body.containsKey("universitySupervisorComment")) {
            diary.setUniversitySupervisorComment(body.get("universitySupervisorComment"));
        }
        DayDiary saved = dayDiaryRepository.save(diary);
    ...
    private Map<String, Object> toView(DayDiary d) {
        ...
        view.put("feedback", d.getSupervisorFeedback());
