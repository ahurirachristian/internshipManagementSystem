import { useEffect, useMemo, useState } from 'react';
import {
  Pencil,
  Trash2,
  Plus,
  Link2,
  X,
  AlertCircle,
  FileText,
  Search,
} from 'lucide-react';
import {
  fetchUniversityUnitCourses,
  createUniversityUnitCourse,
  updateUniversityUnitCourse,
  deleteUniversityUnitCourse,
  fetchUniversityAcademicUnits,
  fetchUniversityCourses,
} from '../services/api';
import ExportButton from './ExportButton';
import Pagination from './Pagination';

const ITEMS_PER_PAGE = 10;

const initialForm = {
  unitId: '',
  courseId: '',
};

export default function UnitCoursesManagement() {
  const [links, setLinks] = useState([]);
  const [units, setUnits] = useState([]);
  const [courses, setCourses] = useState([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [modalOpen, setModalOpen] = useState(false);
  const [form, setForm] = useState(initialForm);
  const [editingId, setEditingId] = useState(null);
  const [searchQuery, setSearchQuery] = useState('');
  const [currentPage, setCurrentPage] = useState(1);

  const unitById = useMemo(() => {
    const map = {};
    units.forEach((unit) => { map[unit.unitId] = unit; });
    return map;
  }, [units]);

  const courseById = useMemo(() => {
    const map = {};
    courses.forEach((course) => { map[course.courseId] = course; });
    return map;
  }, [courses]);

  const filteredLinks = useMemo(() => {
    const q = searchQuery.toLowerCase().trim();
    if (!q) return links;
    return links.filter((link) => {
      const unitName = (unitById[link.unitId]?.unitName || '').toLowerCase();
      const courseName = (courseById[link.courseId]?.courseName || '').toLowerCase();
      return (
        unitName.includes(q) ||
        (unitById[link.unitId]?.shortForm || '').toLowerCase().includes(q) ||
        courseName.includes(q)
      );
    });
  }, [links, searchQuery, unitById, courseById]);

  const totalPages = Math.ceil(filteredLinks.length / ITEMS_PER_PAGE);

  const paginatedLinks = useMemo(() => {
    const start = (currentPage - 1) * ITEMS_PER_PAGE;
    return filteredLinks.slice(start, start + ITEMS_PER_PAGE);
  }, [filteredLinks, currentPage]);

  useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery]);

  useEffect(() => {
    refresh();
  }, []);

  async function refresh() {
    setLoading(true);
    setError('');
    try {
      const [linksData, unitsData, coursesData] = await Promise.all([
        fetchUniversityUnitCourses(),
        fetchUniversityAcademicUnits(),
        fetchUniversityCourses(),
      ]);
      setLinks(Array.isArray(linksData) ? linksData : []);
      setUnits(Array.isArray(unitsData) ? unitsData : []);
      setCourses(Array.isArray(coursesData) ? coursesData : []);
    } catch (err) {
      setLinks([]);
      setError(err.message || 'Unable to load unit-course links.');
    } finally {
      setLoading(false);
    }
  }

  function openModal(existingLink) {
    setEditingId(existingLink?.id ?? null);
    setForm(existingLink ? {
      unitId: existingLink.unitId != null ? String(existingLink.unitId) : '',
      courseId: existingLink.courseId != null ? String(existingLink.courseId) : '',
    } : initialForm);
    setModalOpen(true);
  }

  function closeModal() {
    setModalOpen(false);
    setEditingId(null);
    setForm(initialForm);
  }

  function handleFieldChange(name, value) {
    setForm((prev) => ({ ...prev, [name]: value }));
  }

  const unitOptions = useMemo(
    () => [...units].sort((a, b) => a.unitName.localeCompare(b.unitName)),
    [units]
  );

  const courseOptions = useMemo(
    () => [...courses].sort((a, b) => a.courseName.localeCompare(b.courseName)),
    [courses]
  );

  async function handleSubmit(event) {
    event.preventDefault();
    setError('');

    if (!form.unitId) {
      setError('Please select an academic unit.');
      return;
    }
    if (!form.courseId) {
      setError('Please select a course.');
      return;
    }
    const duplicate = links.find((link) =>
      link.unitId === parseInt(form.unitId, 10) &&
      link.courseId === parseInt(form.courseId, 10) &&
      link.id !== editingId
    );
    if (duplicate) {
      setError('This course is already linked to this unit.');
      return;
    }

    setLoading(true);
    try {
      const payload = {
        unitId: parseInt(form.unitId, 10),
        courseId: parseInt(form.courseId, 10),
      };

      if (editingId) {
        await updateUniversityUnitCourse(editingId, payload);
      } else {
        await createUniversityUnitCourse(payload);
      }

      closeModal();
      await refresh();
    } catch (err) {
      setError(err.message || 'Unable to save unit-course link.');
    } finally {
      setLoading(false);
    }
  }

  async function handleDelete(link) {
    const unitName = unitById[link.unitId]?.unitName || `Unit ${link.unitId}`;
    const courseName = courseById[link.courseId]?.courseName || `Course ${link.courseId}`;
    if (!window.confirm(`Remove the link between "${unitName}" and "${courseName}"?`)) return;
    setLoading(true);
    setError('');
    try {
      await deleteUniversityUnitCourse(link.id);
      await refresh();
    } catch (err) {
      setError(err.message || 'Unable to delete unit-course link.');
    } finally {
      setLoading(false);
    }
  }

  const selectClass = "w-full bg-white text-slate-900 text-xs rounded-xl border border-slate-300 px-3.5 py-2.5 focus:border-teal-600 focus:ring-2 focus:ring-teal-600/20 focus:outline-none transition-all shadow-xs font-medium";
  const labelClass = "block text-xs font-bold uppercase tracking-wider text-slate-800 mb-1.5";

  return (
    <div className="space-y-6 max-w-7xl mx-auto">

      {/* Page Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <div className="flex items-center gap-2 text-[11px] font-semibold text-teal-700/70 uppercase tracking-wider mb-1.5">
            <span>Settings</span>
            <span className="text-slate-400">/</span>
            <span>Unit Courses</span>
          </div>
        </div>
        <div className="flex flex-col sm:flex-row sm:items-center gap-2.5 w-full sm:w-auto">
          <div className="relative flex-1 sm:flex-none">
            <Search className="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" />
            <input
              type="text"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="Search links..."
              className="w-full sm:w-52 bg-white text-slate-900 text-xs rounded-xl border border-slate-300 pl-9 pr-3 py-2 focus:border-teal-600 focus:ring-2 focus:ring-teal-600/20 focus:outline-none transition-all shadow-xs font-medium placeholder:text-slate-400"
              aria-label="Search unit-course links"
            />
          </div>
          <div className="flex items-center gap-2.5">
            <ExportButton data={links} fileName="unit-courses" exportUrl="/api/university/unit-courses/export/csv" />
            <button
              type="button"
              onClick={() => openModal(null)}
              className="h-9 px-3.5 py-2 rounded-xl bg-[#063b33] hover:bg-[#042823] text-white text-xs font-bold shadow-xs transition-all flex items-center gap-1.5 focus-visible:ring-2 focus-visible:ring-teal-600 focus-visible:outline-none"
            >
              <Plus className="w-4 h-4" />
              <span>Add Link</span>
            </button>
          </div>
        </div>
      </div>

      {/* Error Banner */}
      {error && (
        <div role="alert" className="p-3.5 bg-rose-50 border border-rose-200 rounded-xl flex items-center justify-between gap-3 text-rose-900 text-sm animate-in fade-in">
          <div className="flex items-center gap-2.5">
            <AlertCircle className="w-5 h-5 text-rose-600 shrink-0" />
            <span className="font-medium">{error}</span>
          </div>
          <button type="button" onClick={() => setError('')} className="text-rose-600 hover:text-rose-900 p-1 rounded" aria-label="Dismiss error">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}

      {/* Loading */}
      {loading && (
        <div className="flex items-center gap-2 text-xs text-slate-500 font-medium">
          <div className="w-4 h-4 border-2 border-teal-600 border-t-transparent rounded-full animate-spin" />
          <span>Loading unit-course links...</span>
        </div>
      )}

      {/* Table */}
      <section id="unit-courses-table-container" aria-labelledby="unit-courses-table-title" className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-left border-collapse" style={{ minWidth: '750px' }} aria-label="Unit-course links list">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50/90 text-[11px] font-bold tracking-wider text-slate-800">
                <th scope="col" className="py-3.5 px-3 pl-5">ID</th>
                <th scope="col" className="py-3.5 px-3">Academic Unit</th>
                <th scope="col" className="py-3.5 px-3">Course</th>
                <th scope="col" className="py-3.5 px-3">Level</th>
                <th scope="col" className="py-3.5 px-3 pr-5 text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 text-sm">
              {paginatedLinks.length > 0 ? paginatedLinks.map((link) => (
                <tr key={link.id} className="hover:bg-slate-50/80 transition-colors">
                  <td className="py-3.5 px-3 pl-5 text-xs font-medium text-slate-500">{link.id}</td>
                  <td className="py-3.5 px-3">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg bg-blue-50 text-blue-700 border border-blue-200 flex items-center justify-center shrink-0 shadow-xs">
                        <Link2 className="w-4 h-4" />
                      </div>
                      <div>
                        <div className="font-bold text-slate-900">
                          {unitById[link.unitId]?.unitName || `Unit ${link.unitId}`}
                        </div>
                        {unitById[link.unitId]?.shortForm && (
                          <div className="text-[11px] text-slate-500">{unitById[link.unitId].shortForm}</div>
                        )}
                      </div>
                    </div>
                  </td>
                  <td className="py-3.5 px-3 text-xs font-semibold text-slate-700">
                    {courseById[link.courseId]?.courseName || `Course ${link.courseId}`}
                  </td>
                  <td className="py-3.5 px-3">
                    {courseById[link.courseId]?.level ? (
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-50 text-blue-800 border border-blue-200">
                        {courseById[link.courseId].level}
                      </span>
                    ) : (
                      <span className="text-xs text-slate-400">—</span>
                    )}
                  </td>
                  <td className="py-3.5 px-3 pr-5 text-right">
                    <ul className="flex items-center justify-end gap-1 list-none p-0 m-0">
                      <li>
                        <button
                          type="button"
                          onClick={() => openModal(link)}
                          className="p-1.5 text-emerald-600 hover:text-emerald-800 hover:bg-emerald-50 rounded-lg transition-colors focus-visible:ring-2 focus-visible:ring-emerald-600 focus-visible:outline-none"
                          aria-label={`Edit link ${link.id}`}
                          title="Edit"
                        >
                          <Pencil className="w-4 h-4" />
                        </button>
                      </li>
                      <li>
                        <button
                          type="button"
                          onClick={() => handleDelete(link)}
                          className="p-1.5 text-rose-600 hover:text-rose-800 hover:bg-rose-50 rounded-lg transition-colors focus-visible:ring-2 focus-visible:ring-rose-600 focus-visible:outline-none"
                          aria-label={`Delete link ${link.id}`}
                          title="Delete"
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </li>
                    </ul>
                  </td>
                </tr>
              )) : (
                <tr>
                  <td colSpan={5} className="py-12 px-4 text-center">
                    <div className="max-w-sm mx-auto flex flex-col items-center">
                      <div className="w-12 h-12 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-400 mb-3">
                        {searchQuery ? <Search className="w-6 h-6" /> : <FileText className="w-6 h-6" />}
                      </div>
                      <h3 className="text-base font-bold text-slate-800">
                        {searchQuery ? 'No results found' : 'No unit-course links found'}
                      </h3>
                      <p className="text-xs text-slate-500 mt-1 mb-4">
                        {searchQuery ? (
                          <>No links match &ldquo;<strong>{searchQuery}</strong>&rdquo;. Try a different search term.</>
                        ) : (
                          <>Click &ldquo;Add Link&rdquo; to attach a course to an academic unit.</>
                        )}
                      </p>
                      {!searchQuery && (
                        <button
                          type="button"
                          onClick={() => openModal(null)}
                          className="px-3.5 py-2 bg-[#063b33] hover:bg-[#042823] text-white font-bold text-xs rounded-xl transition-colors shadow-xs focus-visible:ring-2 focus-visible:ring-teal-600 focus-visible:outline-none"
                        >
                          Add Link
                        </button>
                      )}
                    </div>
                  </td>
                </tr>
              )}
            </tbody>
          </table>
        </div>

        {/* Pagination */}
        {filteredLinks.length > ITEMS_PER_PAGE && (
          <div className="px-5 py-3 border-t border-slate-200 bg-slate-50/90 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-600">
            <span>
              Showing {((currentPage - 1) * ITEMS_PER_PAGE) + 1}–{Math.min(currentPage * ITEMS_PER_PAGE, filteredLinks.length)} of {filteredLinks.length} links
            </span>
            <Pagination currentPage={currentPage} setCurrentPage={setCurrentPage} totalPages={totalPages} />
          </div>
        )}
      </section>

      {/* Modal */}
      {modalOpen && (
        <div
          role="dialog"
          aria-modal="true"
          aria-labelledby="unit-course-modal-title"
          className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-150"
          onClick={closeModal}
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="bg-white w-full max-w-lg rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-150"
          >
            {/* Modal Header */}
            <div className="p-4 sm:p-5 bg-slate-50 border-b border-slate-200 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-10 h-10 rounded-xl bg-blue-50 border border-blue-200 flex items-center justify-center text-blue-800 shrink-0">
                  <Link2 className="w-5 h-5" />
                </div>
                <h3 id="unit-course-modal-title" className="text-base font-bold text-slate-900">
                  {editingId ? 'Edit Unit-Course Link' : 'Add Unit-Course Link'}
                </h3>
              </div>
              <button
                type="button"
                onClick={closeModal}
                className="p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-200 rounded-lg transition-colors focus-visible:ring-2 focus-visible:ring-teal-600 focus-visible:outline-none"
                aria-label="Close modal"
              >
                <X className="w-5 h-5" />
              </button>
            </div>

            {/* Modal Form */}
            <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
              <div className="p-6 space-y-5 overflow-y-auto flex-1">
                <div>
                  <label htmlFor="uc-unitId" className={labelClass}>
                    Academic Unit <span className="text-rose-600" aria-hidden="true">*</span>
                  </label>
                  <select
                    id="uc-unitId"
                    value={form.unitId}
                    onChange={(e) => handleFieldChange('unitId', e.target.value)}
                    required
                    className={selectClass}
                  >
                    <option value="">Select Unit</option>
                    {unitOptions.map((unit) => (
                      <option key={unit.unitId} value={unit.unitId}>{unit.unitName}</option>
                    ))}
                  </select>
                </div>
                <div>
                  <label htmlFor="uc-courseId" className={labelClass}>
                    Course <span className="text-rose-600" aria-hidden="true">*</span>
                  </label>
                  <select
                    id="uc-courseId"
                    value={form.courseId}
                    onChange={(e) => handleFieldChange('courseId', e.target.value)}
                    required
                    className={selectClass}
                  >
                    <option value="">Select Course</option>
                    {courseOptions.map((course) => (
                      <option key={course.courseId} value={course.courseId}>
                        {course.courseName}{course.level ? ` (${course.level})` : ''}
                      </option>
                    ))}
                  </select>
                  <p className="text-[11px] text-slate-500 mt-1">Only your university's units and courses are listed.</p>
                </div>
              </div>

              {/* Modal Footer */}
              <div className="p-4 sm:p-5 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3">
                <div />
                <div className="flex items-center gap-2.5 w-full sm:w-auto">
                  <button
                    type="button"
                    onClick={closeModal}
                    className="flex-1 sm:flex-none px-3.5 py-2 rounded-xl bg-white hover:bg-slate-100 text-slate-700 text-xs font-semibold border border-slate-300 transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={loading}
                    className="flex-1 sm:flex-none px-4 py-2 rounded-xl bg-[#063b33] hover:bg-[#042823] text-white text-xs font-bold transition-all flex items-center justify-center gap-1.5 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {loading ? 'Saving...' : editingId ? 'Save Changes' : 'Create Link'}
                  </button>
                </div>
              </div>
            </form>
          </div>
        </div>
      )}
    </div>
  );
}
