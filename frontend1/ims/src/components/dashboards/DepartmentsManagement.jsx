...
export default function DepartmentsManagement() {
  ...
  async function load() {
    setLoading(true);
    try {
      const [depts, schs] = await Promise.all([fetchDepartments(), fetchSchools()]);
  ...
  async function handleDelete(id) {
    if (!window.confirm('Delete this department? Programmes will be unlinked.')) return;
    try {
  ...
  function handleExport() {
    const header = 'DepartmentID,SchoolID,DepartmentName';
    const rows = filteredDepartments.map((d) =>
      [d.departmentId, d.schoolId, d.departmentName].join(',')
    );
